package com.example.es.serviceimpl;
...
public class TestServiceImpl implements TestService {
    ...
    @Override
    public List<TestBean> queryByage(String age) {
        return testDao.queryByage(age);
    }

    @Override
    public List<TestBean> findByDescLike(String text) {
        return testDao.findByDescLike(text);
    }

    @Override
    public List<TestBean> findByAgeAndName(Integer age,String name) {
        return testDao.findByAgeAndName(age,name);
    }

    @Override
    public List<TestBean> findByAgeOrName(Integer age,String name) {
        return testDao.findByAgeOrName(age,name);
    }

    /**
     * 组合条件查询
     * 基于原生Rest Client(HighRestClient)
     */
    @Override
    public List<Map<String,Object>> findByTestBean(TestBean testBean) {
        ...
        if (testBean.getId()!=0){
            queryBuilder.must(QueryBuilders.matchQuery("id", testBean.getId()));
        }
        if (testBean.getAge()!=null){
            queryBuilder.must(QueryBuilders.matchQuery("age", testBean.getAge()));
        }
        if (testBean.getDesc()!=null){
            queryBuilder.must(QueryBuilders.matchQuery("desc", testBean.getDesc()));
        }
        if (testBean.getName()!=null){
            queryBuilder.must(QueryBuilders.matchQuery("name", testBean.getName()));
        }
        if (testBean.getSex()!=null){
            queryBuilder.must(QueryBuilders.matchQuery("sex", testBean.getSex()));
        }
    ...
    public List<TestBean> findByDesc(String desc) {
        ...
        List<org.springframework.data.elasticsearch.core.SearchHit<TestBean>> result = testDao.findByDesc(desc);
        for (org.springframework.data.elasticsearch.core.SearchHit<TestBean> testBeanSearchHit : result) {
            Map<String, List<String>> highlightFields = testBeanSearchHit.getHighlightFields();
            //获得内容
            TestBean content = testBeanSearchHit.getContent();
            List<String> descHighlight = highlightFields.get("desc");
            for (String s : descHighlight) {
                //替换为高亮
                content.setDesc(s);
                list.add(content);
            }
     ...
     */
    @Override
    public List<Map<String, Object>> highlighted(String desc) {
        //定义索引库
        SearchRequest searchRequest = new SearchRequest("testdoct");
        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
        //定义query查询
        ...
        try {
            ...
            for (SearchHit hit : searchResponse.getHits()) {
                ...
                HighlightField nameHighlight = highlightFields.get("desc");
                Map<String, Object> sourceAsMap = hit.getSourceAsMap();
