...
《黑马代码生成器》一款由传智播客教育集团深圳校区JavaEE教研团队开发的基于Freemarker模板引擎的“代码生成神器”。即便是一个工程几百个表，也可以瞬间完成基础代码的构建！用户只需建立数据库表结构，运行main方法就可快速生成可以运行的一整套代码，可以极大地缩短开发周期，降低人力成本。《黑马代码生成器》的诞生主要用于迅速构建生成微服务工程的Pojo、Dao、Service、Controller各层、并且可以生成swagger API模板等。 用户通过自己开发模板也可以实现生成php、python、C# 、c++、数据库存储过程等其它编程语言的代码。
...
品牌与分类进行关联，我们这里通过品牌与分类的关联表实现，表结构如下：
...
changgou-service-goods-api工程中的com.changgou.goods.pojo.CategoryBrand实体类，代码如下：
...
修改changgou-service-goods工程，导入com.changgou.goods.dao.CategoryBrandMapper接口，代码如下：
...
修改changgou-service-goods工程，修改com.changgou.goods.service.impl.SpuServiceImpl类，注入`CategoryBrandMapper`,并添加代码实现分类与品牌的关联，代码如下：

![1564352542006](image\1564352542006.png)

注入CategoryBrandMapper代码如下：
